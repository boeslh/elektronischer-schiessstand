...
CREATE TABLE shots (
    ...
    raw_t_us       INTEGER[] NOT NULL,        -- TDOA-Timestamps [0,231,584,402]
    ...
    -- ---- ROHDATEN Firmware Rev 4.6.1+ (Migration 013) ----
    -- Positionsberechnung inkl. Kalibrierung läuft auf dem ESP32 selbst;
    -- x_um/y_um kommen fertig im Telegramm, air_ns/Qualitätswerte werden
    -- zusätzlich für spätere Analysen/Simulationen gespeichert. reject-
    -- Telegramme und shot-Telegramme mit pos_valid=0 landen ebenfalls hier
    -- (status='rejected'), nur nicht in Anzeige/Wertung.
    air_ns         JSONB,                     -- alle Flanken je Mikrofon (ns)
    x_um           BIGINT,                    -- Geräteseitige Position, 0.001mm
    y_um           BIGINT,
    pos_res_um     BIGINT,                    -- Rest-Fehler Stufe 1, 0.001mm
    precision_um   BIGINT,                    -- RMS Kandidaten-Cluster, 0.001mm
    cluster_hits   SMALLINT,
    pos_valid      BOOLEAN,
    piezo_ns       BIGINT,                    -- NULL wenn Piezo nicht ausgelöst
    piezo_ok       BOOLEAN,                   -- NULL wenn SET PIEZO=0
    clean          BOOLEAN,                   -- Qualitätsschwellen erfüllt (nur type=shot)
    reject_reason  TEXT,                      -- "reason" aus reject-Telegramm bzw. pos_valid=0
    device_ts_ms   BIGINT,                    -- "ts" aus dem Telegramm (Geräte-Uptime)
...
VALUES ('00000000-0000-0000-0000-00000000la10',
        'LG 10m ISSF', 80, 80, 0.5, 4.5, TRUE)
ON CONFLICT DO NOTHING;

INSERT INTO target_rings (target_id, ring_value, diameter_mm) VALUES
('00000000-0000-0000-0000-00000000la10', 10,  0.5),
('00000000-0000-0000-0000-00000000la10',  9,  5.5),
('00000000-0000-0000-0000-00000000la10',  8, 10.5),
('00000000-0000-0000-0000-00000000la10',  7, 15.5),
('00000000-0000-0000-0000-00000000la10',  6, 20.5),
('00000000-0000-0000-0000-00000000la10',  5, 25.5),
('00000000-0000-0000-0000-00000000la10',  4, 30.5),
('00000000-0000-0000-0000-00000000la10',  3, 35.5),
('00000000-0000-0000-0000-00000000la10',  2, 40.5),
('00000000-0000-0000-0000-00000000la10',  1, 45.5)
ON CONFLICT DO NOTHING;

-- LP-Scheibe 10m (ISSF): Zehner 11,5mm, Ringabstand 8mm radial
INSERT INTO targets (id, name, card_width_mm, card_height_mm,
                     inner_ten_d_mm, caliber_mm, edge_scoring)
VALUES ('00000000-0000-0000-0000-00000000lp10',
        'LP 10m ISSF', 170, 170, 5.0, 4.5, TRUE)
ON CONFLICT DO NOTHING;

INSERT INTO target_rings (target_id, ring_value, diameter_mm) VALUES
('00000000-0000-0000-0000-00000000lp10', 10,  11.5),
('00000000-0000-0000-0000-00000000lp10',  9,  27.5),
('00000000-0000-0000-0000-00000000lp10',  8,  43.5),
('00000000-0000-0000-0000-00000000lp10',  7,  59.5),
('00000000-0000-0000-0000-00000000lp10',  6,  75.5),
('00000000-0000-0000-0000-00000000lp10',  5,  91.5),
('00000000-0000-0000-0000-00000000lp10',  4, 107.5),
('00000000-0000-0000-0000-00000000lp10',  3, 123.5),
('00000000-0000-0000-0000-00000000lp10',  2, 139.5),
('00000000-0000-0000-0000-00000000lp10',  1, 155.5)
ON CONFLICT DO NOTHING;
...
('Luftgewehr 40 Schuss',  '1.10', 10,
 '00000000-0000-0000-0000-00000000la10', 40, 10, FALSE, 4500, TRUE),
('Luftpistole 40 Schuss', '2.10', 10,
 '00000000-0000-0000-0000-00000000lp10', 40, 10, FALSE, 4500, TRUE)
ON CONFLICT DO NOTHING;
